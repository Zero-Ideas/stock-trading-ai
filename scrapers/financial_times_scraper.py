#!/usr/bin/env python3
"""
Financial Times scraper for stock sentiment analysis
"""

from typing import List
import random
from datetime import datetime, timedelta
from urllib.parse import quote
from bs4 import BeautifulSoup
from .base_scraper import BaseScraper, SentimentData


class FinancialTimesScraper(BaseScraper):
    """Scraper for Financial Times articles"""
    
    @property
    def source_name(self) -> str:
        return "Financial Times"
    
    def _analyze_text(self, text: str) -> dict:
        """Analyze text sentiment (placeholder - will be handled by main analyzer)"""
        # This is a placeholder - actual sentiment analysis will be done by the main StockSentimentAnalyzer
        return {'polarity': 0.0, 'compound': 0.0}
    
    def scrape(self, max_articles: int = 8) -> List[SentimentData]:
        """Scrape Financial Times for stock news"""
        sentiments = []
        
        try:
            # Financial Times search URL
            search_url = f"https://www.ft.com/search?q={quote(self.symbol)}"
            
            try:
                response = self.make_request(search_url, timeout=15)
                if response.status_code != 200:
                    return sentiments
                    
                soup = BeautifulSoup(response.content, 'html.parser')
                
                # Try multiple selectors for articles
                article_selectors = [
                    'div.o-teaser',
                    'article.o-teaser',
                    'div[data-trackable="story-list-item"]',
                    'div.story-item',
                    'article'
                ]
                
                articles = []
                for selector in article_selectors:
                    found = soup.select(selector)[:max_articles]
                    if found:
                        articles = found
                        break
                
                for article in articles:
                    if len(sentiments) >= max_articles:
                        break
                        
                    try:
                        title_elem = (article.find('h3') or 
                                    article.find('h2') or 
                                    article.find('h4') or
                                    article.find('a'))
                        if not title_elem:
                            continue
                            
                        title = title_elem.get_text(strip=True)
                        if not title or len(title) < 10:
                            continue
                        
                        # Check relevance
                        if not self.is_relevant_content(title):
                            continue
                        
                        # Get summary
                        summary_elem = article.find('p')
                        summary = summary_elem.get_text(strip=True) if summary_elem else ""
                        
                        text = f"{title}. {summary}" if summary else title
                        text = self.clean_text(text)
                        
                        # Get URL
                        link_elem = article.find('a')
                        url = ""
                        if link_elem and link_elem.get('href'):
                            href = link_elem.get('href')
                            if href.startswith('/'):
                                url = f"https://www.ft.com{href}"
                            elif href.startswith('http'):
                                url = href
                        
                        sentiment = self._analyze_text(text)
                        sentiments.append(SentimentData(
                            text=text,
                            source="Financial Times",
                            timestamp=datetime.now() - timedelta(hours=random.randint(1, 48)),
                            polarity=sentiment['polarity'],
                            compound=sentiment['compound'],
                            url=url
                        ))
                        
                    except Exception as e:
                        if self.debug:
                            print(f"Error processing Financial Times article: {e}")
                        continue
                        
            except Exception as e:
                if self.debug:
                    print(f"Error with Financial Times search: {e}")
        
        except Exception as e:
            if self.debug:
                print(f"Financial Times scraping error: {e}")
        
        return sentiments